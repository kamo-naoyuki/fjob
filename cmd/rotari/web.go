...
function enhancePage(){document.querySelectorAll('.web-copy-controls,.web-queue-commands,.web-origin').forEach(e=>e.remove());const parts=location.pathname.split('/').filter(Boolean);if(parts[0]!=='queue')return;const queue=state.queues.find(q=>q.queue_name===decodeURIComponent(parts[1]));if(!queue)return;if(parts[2]==='run'){const runID=decodeURIComponent(parts[3]);const controls=document.createElement('div');controls.className='toolbar web-copy-controls';controls.innerHTML='<button onclick="copyRun(\''+esc(queue.queue_name)+'\',\''+esc(runID)+'\',\'failed\')">Copy failed jobs</button><button onclick="copyRun(\''+esc(queue.queue_name)+'\',\''+esc(runID)+'\',\'all\')">Copy all jobs</button>';document.getElementById('app').prepend(controls);return}const commands=queue.queue.commands||[];const section=document.createElement('section');section.className='web-queue-commands';section.innerHTML='<h2>Current queue</h2>'+(commands.length?'<table class="runs web-queue-jobs"><thead><tr><th data-sort="name">Job name / ID</th><th data-sort="status">Status</th><th data-sort="executor">Executor</th><th data-sort="options">Executor options</th><th data-sort="depends">Dependencies</th><th data-sort="command">Command</th></tr></thead><tbody>'+commands.map(j=>'<tr><td><strong>'+esc(j.name||'-')+'</strong><div class="meta">'+esc(j.id)+'</div></td><td>pending</td><td>'+esc(j.executor||'default')+'</td><td>'+esc((j.executor_options||[]).join(' ')||'-')+'</td><td>'+esc((j.depends_on||[]).join(', ')||'-')+'</td><td class="command">'+esc((j.command||[]).join(' '))+'</td></tr>').join('')+'</tbody></table>':'<div class="empty">Queue is empty.</div>');document.getElementById('app').prepend(section);fixQueueSourceColumns(commands)}
