...
func cmdRerun(args []string) int {
	...
	if len(jobIDs) > 0 {
		if selection != "" {
			fmt.Fprintln(os.Stderr, "--job-id may not be used with --failed, --unfinished, --success, or --nonsuccess")
			return 1
		}
		selection = "job-id"
	}
	if len(left) != 0 || *localConcurrency < 1 || *batchConcurrency < 1 || *retry < -1 {
		fmt.Fprintln(os.Stderr, "usage: "+cliUsage("rerun"))
		return 1
	}
	baseDir, _, err := resolveBaseDir(*basedir)
	if err != nil {
	...
	if selection != "" {
		...
		if metaErr != nil {
			...
			message, copyErr := copyRunToQueue(baseDir, queueName, meta.LastRunID, selection, jobIDs, false, overwriteConfirmed)
			if copyErr != nil {
...
func (server *fjobServer) handle(baseDir string, conn net.Conn) {
	...
	case "run":
		if request.Selection != "" {
			if _, err := prepareRunSelection(baseDir, request.QueueName, request.Selection, request.JobIDs); err != nil {
				response = serverResponse{Message: err.Error()}
				break
			}
		}
		var message string
		...
		if request.Async {
			...
			message, err = startServerRun(baseDir, request.QueueName, request.RunName, request.LocalConcurrency, request.BatchMaxActive, request.Retry, request.Executor, request.ExecutorOptions, onDone, request.CWD)
			...
			server.beginRun()
			defer server.endRun()
			message, exitCode, err = runServerSyncWithDisconnect(conn, baseDir, request.QueueName, request.RunName, request.LocalConcurrency, request.BatchMaxActive, request.Retry, request.Executor, request.ExecutorOptions, func(progress serverResponse) {
...
func runServerSyncWithDisconnect(conn net.Conn, baseDir, queueName, runName string, localConcurrency, batchMaxActive, retry int, executor string, executorOptions []string, progress func(serverResponse), cwdOverride ...string) (string, int, error) {
	cwd := ""
	...
	go func() {
		message, exitCode, err := runServerSync(baseDir, queueName, runName, localConcurrency, batchMaxActive, retry, executor, executorOptions, progress, cwd)
...
func startServerRun(baseDir, queueName, runName string, localConcurrency, batchMaxActive, retry int, executor string, executorOptions []string, onDone func(), cwd string) (string, error) {
	...
	runID := makeRunID()
	if err := launchAsyncRun(paths, queueName, runID, runName, localConcurrency, batchMaxActive, retry, executor, executorOptions, cwd); err != 0 {
...
func runServerSync(baseDir, queueName, runName string, localConcurrency, batchMaxActive, retry int, executor string, executorOptions []string, progress func(serverResponse), cwd string) (string, int, error) {
	...
	exitCode := executeMixedRun(paths, runID, runName, localConcurrency, batchMaxActive, retry, resolvedExecutor, executorOptions, func(result JobResult, completed, total, succeeded, failed int) {
		if progress != nil {
			message := ""
