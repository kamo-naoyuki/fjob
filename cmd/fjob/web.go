...
func webHTML() string {
	...
	template = strings.Replace(template, "fjob copy", "fjob retry", -1)
	template = strings.Replace(template, "\\nfjob rerun'+basedir+' --queue-name '+shellQuote(queueName)+' --job-id JOB_ID", "", -1)
	template = strings.Replace(template, "fjob rerun", "fjob retry", -1)
